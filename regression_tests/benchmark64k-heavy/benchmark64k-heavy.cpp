...
public:
    Benchmark64KExampleHeavy() {
        ...
        for (int i = 0;i<materials.size();i++){
            // explicit use
            if (i % 2==0){
                materials[i]->setTexture(Texture::create().withFile("t_explosionsheet.png").withGenerateMipmaps(true).build());
            } else {
                materials[i]->setTexture(Texture::create().withFile("gamma-test.png").withGenerateMipmaps(true).build());
            }
        }
