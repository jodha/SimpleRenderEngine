...
public:
    ...
    bool initializeEventRecorder(bool& recordingEvents,         // A helper function to initialize the event recording and playback functionality based on the
                          bool& playingEvents,                  // parameters given. The function returns true if successful, false if not. If there is an error,
                          const std::string& eventsFileName,    // then the error message is returned in the errorMessage string.
                          std::string&  errorMessage);
    bool startRecordingEvents(const std::string& fileName,      // Start recording SDL events (e.g. keyboard, mouse, mouse motion, etc.). This is useful
                              std::string&  errorMessage);      // for testing. Recommend only allowing recording right after application has started (e.g.
                                                                ...
                                                                // has been operating for even a short amount of time). Error messages are return in errorMessage.
    ...
    bool startPlayingEvents(const std::string& fileName,        // Play back recorded SDL events. Error messages are return in errorMessage.
                            std::string&  errorMessage);
